    ...
    if config:
        ...
        if config["redis"]["enabled"] and j.core.db:
            ...
            redis_handler = RedisLogHandler(
                ...
                dump=redis_config["dump"],
                dump_dir=redis_config["dump_dir"],
            )
            logger.add_custom_handler(
                "redis",
                redis_handler,
                serialize=True,
                level=redis_config["level"],
            )
        ...
        if config["filesystem"]["enabled"]:
            ...
            logger.add_handler(
                sink=fs_config["log_dir"],
                level=fs_config["level"],
                rotation=fs_config["rotation"],
            )
