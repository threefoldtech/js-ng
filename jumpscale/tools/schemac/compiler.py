...
class Compiler:
    ...
    def parse(self):
        ...
        if to_process.count("@url") == 1:
            ...
            urls_positions = [
                m.start() for m in re.finditer("@url", self._schema_text)
            ]
            urls_positions.append(len(self._schema_text))
            start_end_positions = list(zip(urls_positions, urls_positions[1:]))

            schemas_texts = [
                self._schema_text[start:end]
                for (start, end) in start_end_positions
            ]

        parsed_schemas = [
            j.data.schema.parse_schema(schema_text)
            for schema_text in schemas_texts
        ]
