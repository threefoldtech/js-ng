...
def test_hash_strings():
    ...
    assert (
        ...
        == hashlib.sha224(test_string.encode()).hexdigest()
    )
    assert (
    ...
    assert (
        j.data.hash.sha384(test_string)
        == hashlib.sha384(test_string.encode()).hexdigest()
    )
    assert (
    ...
    assert (
        j.data.hash.sha3_224(test_string)
        == hashlib.sha3_224(test_string.encode()).hexdigest()
    )
    assert (
        j.data.hash.sha3_256(test_string)
        == hashlib.sha3_256(test_string.encode()).hexdigest()
    )
    assert (
        j.data.hash.sha3_384(test_string)
        == hashlib.sha3_384(test_string.encode()).hexdigest()
    )
    assert (
        j.data.hash.sha3_512(test_string)
        == hashlib.sha3_512(test_string.encode()).hexdigest()
    )
    assert (
        j.data.hash.blake2b(test_string)
        == hashlib.blake2b(test_string.encode()).hexdigest()
    )
    assert (
        j.data.hash.blake2s(test_string)
        == hashlib.blake2s(test_string.encode()).hexdigest()
    )
...
def test_hash_bytes():
    ...
    assert (
        j.data.hash.sha1(test_string) == hashlib.sha1(test_string).hexdigest()
    )
    assert (
        j.data.hash.sha224(test_string)
        == hashlib.sha224(test_string).hexdigest()
    )
    assert (
        j.data.hash.sha256(test_string)
    ...
    assert (
        j.data.hash.sha384(test_string)
        == hashlib.sha384(test_string).hexdigest()
    )
    assert (
        j.data.hash.sha512(test_string)
        == hashlib.sha512(test_string).hexdigest()
    )
    assert (
        j.data.hash.sha3_224(test_string)
        == hashlib.sha3_224(test_string).hexdigest()
    )
    assert (
        j.data.hash.sha3_256(test_string)
        == hashlib.sha3_256(test_string).hexdigest()
    )
    assert (
        j.data.hash.sha3_384(test_string)
        == hashlib.sha3_384(test_string).hexdigest()
    )
    assert (
        j.data.hash.sha3_512(test_string)
        == hashlib.sha3_512(test_string).hexdigest()
    )
    assert (
        j.data.hash.blake2b(test_string)
        == hashlib.blake2b(test_string).hexdigest()
    )
    assert (
        j.data.hash.blake2s(test_string)
        == hashlib.blake2s(test_string).hexdigest()
    )
    assert j.data.hash.shake_128(test_string) == hashlib.shake_128(
        test_string
    ).hexdigest(16)
...
def test_hash_files(make_list):
    with tempfile.NamedTemporaryFile("w") as tf:
        ...
        for h in make_list:
            ...
            assert (
                j.data.hash.hash_file(tf.name, h).hexdigest() == ha.hexdigest()
            )
