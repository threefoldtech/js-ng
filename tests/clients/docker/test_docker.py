...
class TestDockerClient(BaseTests):
    ...
    def tearDown(self):
        ...
        j.sals.process.execute(
            'docker rm -f $(docker ps -aqf "name=docker|DOCKER")'
        )
    ...
    def docker_check_status(self, docker_name, status):
        ...
        """
        _, output, _ = j.sals.process.execute(
            'docker inspect {} | grep "Running"'.format(docker_name)
        )
    ...
    def test01_docker_get(self):
        ...
        **Test Scenario**
        ...
        _, output, _ = j.sals.process.execute(
            "docker ps -aqf name={}".format(self.DOCKER_NAME)
        )
    ...
    def test02_docker_list(self):
        ...
        **Test Scenario**
        ...
        DOCKER_ID_2 = self.client.create(
            DOCKER_NAME_2, self.DOCKER_IMAGE
        ).short_id
        self.assertTrue(self.docker_check_status(DOCKER_NAME_2, "false"))

        self.info(
            "Try to list container using list method with option all=False to list only the running container"
        )
        docker_list = self.client.list(all=False)

        self.info("Check the output of list command")
        self.info(
            " it should contain the first docker, but doesn't contain the second one"
        )
        ...
        self.assertIn(
            DOCKER_ID_1,
            str(docker_list),
            "{} doesn't exist in the docker list".format(self.DOCKER_NAME),
        )
        self.assertIn(
            DOCKER_ID_2,
            str(docker_list),
            "{} doesn't exist in the docker list".format(DOCKER_NAME_2),
        )
    ...
    def test05_docker_exec(self):
        ...
        **Test Scenario**
        ...
        execute_command = self.client.exec(
            self.DOCKER_NAME, "touch /tmp/test_exec"
        )
        self.assertIn("exit_code=0", str(execute_command))

        self.info("check that file has been created correctly")
        _, output, _ = j.sals.process.execute(
            "docker exec {}  ls /tmp/".format(self.DOCKER_NAME)
        )
    ...
    def test06_docker_delete(self):
        ...
        **Test Scenario**
        ...
        _, output, _ = j.sals.process.execute(
            'docker ps -a | grep -w "{}"'.format(self.DOCKER_NAME)
        )
        ...
        self.info("Check that the container has been deleted correctly")
        _, output, _ = j.sals.process.execute(
            'docker ps -a | grep -w "{}"'.format(DOCKER_NAME)
        )
        self.assertFalse(output)
